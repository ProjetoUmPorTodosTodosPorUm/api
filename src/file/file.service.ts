...
export class FileService {
  ...
  async create(user: User, file: Express.Multer.File, createFileDto: CreateFileDto) {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      child_process.execSync(`cd files && docker run --rm -v $PWD:/source -v ${filesVolumeName}:/dest -w /source alpine cp "${file.filename}" /dest`)
    }
  ...
  async bulkCreate(user: User, files: Express.Multer.File[], createFilesDto: CreateFileDto) {
    ...
    if (process.env.NODE_ENV === 'development') {
      // check docker-compose.yml file
      const filesVolumeName = 'files'
      child_process.execSync(`cd files && docker run --rm -v $PWD:/source -v ${filesVolumeName}:/dest -w /source alpine cp ${filesObj.map(file => `"${file.name}"`).join(" ")} /dest`)
    }
