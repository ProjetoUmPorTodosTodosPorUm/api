...
export class ChurchController {
	...
	restore(@Body() restoreDto: RestoreDto, @Jwt() user: User) {
		return this.churchService.restore(restoreDto, user)
	}

	@ApiBearerAuth()
	@ApiResponse(CreateChurchDto)
	@Put(':id')
	update(@Param('id') id: string, @Jwt() user: User, @Body() updateChurchDto: UpdateChurchDto) {
	...
	hardRemove(@Body() hardRemoveDto: HardRemoveDto, @Jwt() user: User) {
		return this.churchService.hardRemove(hardRemoveDto, user)
	}
