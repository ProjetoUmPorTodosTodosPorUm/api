...
describe('Church Controller E2E', () => {
	...
	const createChurch = async (name: string, description: string, type: ChurchType, field: string) =>
	...
	describe('Private Routes (as Non Logged User)', () => {
		...
		it('Should Not Update a Church', async () => {
			...
			await request(app.getHttpServer()).put(`${baseRoute}/${church.id}`).send({ name: 'Igreja 1' }).expect(401)
		})

		it('Should Not Remove a Church', async () => {
			const church = await createChurch(name, description, type, field.id)

			await request(app.getHttpServer()).delete(`${baseRoute}/${church.id}`).expect(401)

			const res = await request(app.getHttpServer()).get(`${baseRoute}/${church.id}`).expect(200)
