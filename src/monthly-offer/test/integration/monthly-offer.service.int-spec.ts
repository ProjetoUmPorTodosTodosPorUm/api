...
describe('Monthly Offer Service Integration', () => {
	...
	describe('findOne()', () => {
		...
		it('Should Return a Monthly Offer', async () => {
			const MonthlyOfferCreated = await createMonthlyOffer(month, year, foodQnt, monetaryValue, othersQnt, field.id)

			const MonthlyOffer = await monthlyOfferService.findOne(MonthlyOfferCreated.id)
			expect(MonthlyOffer.month).toBe(month)
			expect(MonthlyOffer.year).toBe(year)
			expect(MonthlyOffer.foodQnt).toBe(foodQnt)
			expect(MonthlyOffer.monetaryValue).toBe(monetaryValue)
	...
	describe('update()', () => {
		it('Should Not Update a Monthly Offer (Not Found as USER)', async () => {
			try {
				...
				expect(error.response.message).toBe(TEMPLATE.EXCEPTION.NOT_FOUND('oferta mensal', 'a'))
			}
		})
		...
		it('Should Update a Monthly Offer (as USER)', async () => {
			...
			const MonthlyOfferUpdated = await monthlyOfferService.update(MonthlyOffer.id, user, {
				foodQnt: newFoodQnt,
			})
		...
		it('Should Update a Monthly Offer (as ADMIN)', async () => {
			...
			const MonthlyOfferUpdated = await monthlyOfferService.update(MonthlyOffer.id, admin, {
				foodQnt: newFoodQnt,
			})
			expect(MonthlyOfferUpdated).toBeDefined()
			expect(MonthlyOfferUpdated.foodQnt).toBe(newFoodQnt)
		...
		it('Should Update a Monthly Offer (as WEB MASTER)', async () => {
			...
			const MonthlyOfferUpdated = await monthlyOfferService.update(MonthlyOffer.id, webMaster, {
				foodQnt: newFoodQnt,
	...
	describe('restore()', () => {
		...
		it('Should Restore a Monthly Offer (as ADMIN)', async () => {
			...
			await monthlyOfferService.restore({ ids: [MonthlyOffer.id] }, admin)
			const isMonthlyOfferRestored = await prisma.monthlyOffer.findFirst({
				where: {
					id: MonthlyOffer.id,
				},
			})
