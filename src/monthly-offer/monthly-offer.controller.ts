...
export class MonthlyOfferController {
  constructor(
    private readonly monthlyOfferService: MonthlyOfferService,
  ) { }
  ...
  create(
    @Jwt() user: User,
    @Body() createMonthlyOfferDto: CreateMonthlyOfferDto,
  ) {
  ...
  update(
    @Param('id') id: string,
    @Jwt() user: User,
    @Body() updateMonthlyOfferDto: UpdateMonthlyOfferDto,
  ) {
    return this.monthlyOfferService.update(
      id,
      user,
      updateMonthlyOfferDto,
    );
  }
