...
export class TestimonialController {
	...
	restore(@Body() restoreDto: RestoreDto, @Jwt() user: User) {
		return this.testimonialService.restore(restoreDto, user)
	}

	@ApiBearerAuth()
	@ApiResponse(CreateTestimonialDto)
	@Put(':id')
	update(@Param('id') id: string, @Jwt() user: User, @Body() updateTestimonialDto: UpdateTestimonialDto) {
	...
	hardRemove(@Body() hardRemoveDto: HardRemoveDto, @Jwt() user: User) {
		return this.testimonialService.hardRemove(hardRemoveDto, user)
	}
