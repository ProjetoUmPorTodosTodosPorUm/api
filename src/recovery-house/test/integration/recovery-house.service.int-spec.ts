...
describe('Recovery House Service Integration', () => {
  ...
  const createRecoveryHouse = async (
    title: string,
    description: string,
    field: string,
  ) =>
  ...
  describe('findOne()', () => {
    ...
    it('Should Return a Recovery House', async () => {
      const recoveryHouseCreated = await createRecoveryHouse(
        title,
        description,
        field.id,
      );

      const recoveryHouse = await recoveryHouseService.findOne(
        recoveryHouseCreated.id,
      );
  ...
  describe('update()', () => {
    it('Should Not Update a Recovery House (Not Found as USER)', async () => {
      try {
        ...
        expect(error.response.message).toBe(
          TEMPLATE.EXCEPTION.NOT_FOUND('casa de recuperação', 'a'),
        );
      }
    });
    ...
    it('Should Not Update a Recovery House (Different Field as USER)', async () => {
      try {
        ...
        const recoveryHouse = await createRecoveryHouse(
          title,
          description,
          differentField.id,
        );
    ...
    it('Should Update a Recovery House (as USER)', async () => {
      ...
      const recoveryHouseUpdated = await recoveryHouseService.update(
        recoveryHouse.id,
        user,
        {
          title: newTitle,
