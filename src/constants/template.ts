export class TEMPLATE {
	...
	static VALIDATION = {
		...
		MIN_LENGTH: (prop: string, value: number) => `${prop} deve ter o tamanho mínimo de ${value} caracteres ou items.`,
		MAX_LENGTH: (prop: string, value: number) => `${prop} deve ter o tamanho máximo de ${value} caracteres ou items.`,
		...
		IS_ENUM: (prop: string, values: string[]) => `${prop} deve ser um dos seguintes valores: ${values}.`,
	...
	static EXCEPTION = {
		NOT_FOUND: (name: string, gender: string) => `${gender.toUpperCase()} ${name} não foi encontrad${gender}.`,
		NOT_FOUND_PLR: (name: string, gender: string) => `${name} não foram encontrad${gender}s.`,
		CONFLICT: (name: string, gender: string) => `${gender.toUpperCase} ${name} já está sendo utilizad${gender}.`,
		FILE_SIZE_EXCEEDS: (maxSize: number) => `O tamanho do arquivo excede o máximo de ${maxSize / 1024 / 1024} Mb.`,
		FILES_SIZE_EXCEEDS: (maxSize: number) =>
			`O tamanho de um dos arquivos excede o máximo de ${maxSize / 1024 / 1024} Mb.`,
	}

	static MAIL = {
		RECOVER_MAIL_URL: (domain: string, email: string, token: string, locale = 'pt-BR') =>
			`${domain}/${locale}/forgot-password/step-three?email=${email}&token=${token}`,
		CREATE_MAIL_URL: (domain: string, email: string, token: string, locale = 'pt-BR') =>
			`${domain}/${locale}/signup/step-three?email=${email}&token=${token}`,
	}

	static PIPES = {
		MAX_FILES_SIZE: (maxSize: number) => `Falha na validação (tamanho esperado é maior que ${maxSize})`,
