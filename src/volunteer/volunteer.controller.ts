...
export class VolunteerController {
	...
	restore(@Body() restoreDto: RestoreDto, @Jwt() user: User) {
		return this.volunteerService.restore(restoreDto, user)
	}

	@ApiBearerAuth()
	@ApiResponse(CreateVolunteerDto)
	@Put(':id')
	update(@Param('id') id: string, @Jwt() user: User, @Body() updateVolunteerDto: UpdateVolunteerDto) {
	...
	hardRemove(@Body() hardRemoveDto: HardRemoveDto, @Jwt() user: User) {
		return this.volunteerService.hardRemove(hardRemoveDto, user)
	}
