...
describe('Volunteer Service Integration', () => {
	...
	describe('findOne()', () => {
		...
		it('Should Return a Volunteer', async () => {
			const volunteerCreated = await createVolunteer(firstName, joinedDate, field.id)
	...
	describe('update()', () => {
		it('Should Not Update Volunteer (Not Found as USER)', async () => {
			try {
				...
				expect(error.response.message).toBe(TEMPLATE.EXCEPTION.NOT_FOUND('voluntário', 'o'))
			}
		})
		...
		it('Should Update Volunteer (as USER)', async () => {
			...
			const volunteerUpdated = await volunteerService.update(volunteer.id, user, {
				firstName: newFirstName,
			})
		...
		it('Should Update Volunteer (as ADMIN)', async () => {
			...
			const volunteerUpdated = await volunteerService.update(volunteer.id, admin, {
				firstName: newFirstName,
			})
			expect(volunteerUpdated).toBeDefined()
			expect(volunteerUpdated.firstName).toBe(newFirstName)
		...
		it('Should Update Volunteer (as WEB MASTER)', async () => {
			...
			const volunteerUpdated = await volunteerService.update(volunteer.id, webMaster, {
				firstName: newFirstName,
