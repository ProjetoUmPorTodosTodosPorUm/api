...
describe('Collaborator Service Integration', () => {
	...
	const createCollaborator = async (title: string, description: string, field: string) =>
	...
	describe('findOne()', () => {
		...
		it('Should Return a Collaborator', async () => {
			const collaboratorCreated = await createCollaborator(title, description, field.id)

			const collaborator = await collaboratorService.findOne(collaboratorCreated.id)
	...
	describe('update()', () => {
		it('Should Not Update a Collaborator (Not Found as USER)', async () => {
			try {
				...
				expect(error.response.message).toBe(TEMPLATE.EXCEPTION.NOT_FOUND('colaborador', 'o'))
			}
		})
		...
		it('Should Update a Collaborator (as USER)', async () => {
			const collaborator = await createCollaborator(title, description, field.id)
			const newTitle = 'Abreu'

			const collaboratorUpdated = await collaboratorService.update(collaborator.id, user, {
				title: newTitle,
			})
		...
		it('Should Update a Collaborator (as ADMIN)', async () => {
			...
			const collaboratorUpdated = await collaboratorService.update(collaborator.id, admin, {
				title: newTitle,
			})
			expect(collaboratorUpdated).toBeDefined()
			expect(collaboratorUpdated.title).toBe(newTitle)
		...
		it('Should Update a Collaborator (as WEB MASTER)', async () => {
			...
			const collaboratorUpdated = await collaboratorService.update(collaborator.id, webMaster, {
				title: newTitle,
