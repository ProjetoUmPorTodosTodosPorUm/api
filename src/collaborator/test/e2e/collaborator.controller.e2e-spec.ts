...
describe('Collaborator Controller E2E', () => {
	...
	describe('Private Routes (as Non Logged User)', () => {
		...
		it('Should Not Update a Collaborator', async () => {
			...
			await request(app.getHttpServer()).put(`${baseRoute}/${collaborator.id}`).send({ title: 'Mario' }).expect(401)
		})

		it('Should Not Remove a Collaborator', async () => {
			const collaborator = await createCollaborator(title, description, field.id)

			await request(app.getHttpServer()).delete(`${baseRoute}/${collaborator.id}`).expect(401)

			const res = await request(app.getHttpServer()).get(`${baseRoute}/${collaborator.id}`).expect(200)
