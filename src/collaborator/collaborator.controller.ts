...
export class CollaboratorController {
	...
	restore(@Body() restoreDto: RestoreDto, @Jwt() user: User) {
		return this.collaboratorService.restore(restoreDto, user)
	}

	@ApiBearerAuth()
	@ApiResponse(CreateCollaboratorDto)
	@Put(':id')
	update(@Param('id') id: string, @Jwt() user: User, @Body() updateCollaboratorDto: UpdateCollaboratorDto) {
	...
	hardRemove(@Body() hardRemoveDto: HardRemoveDto, @Jwt() user: User) {
		return this.collaboratorService.hardRemove(hardRemoveDto, user)
	}
