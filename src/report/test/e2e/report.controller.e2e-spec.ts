...
describe('Report Controller E2E', () => {
	...
	describe('Private Routes (as Non Logged User)', () => {
		...
		it('Should Not Update a Report', async () => {
			const report = await createReport(title, shortDescription, month, year, field.id)
			await request(app.getHttpServer()).put(`${baseRoute}/${report.id}`).send({ title: 'Novo Título' }).expect(401)
		})

		it('Should Not Remove a Report', async () => {
			const report = await createReport(title, shortDescription, month, year, field.id)
			await request(app.getHttpServer()).delete(`${baseRoute}/${report.id}`).expect(401)

			const res = await request(app.getHttpServer()).get(`${baseRoute}/${report.id}`).expect(200)
	...
	describe('Private Routes (as Logged VOLUNTEER)', () => {
		...
		it('Should Not Update a Report (Different Field)', async () => {
			...
			const report = await createReport(title, shortDescription, month, year, differentField.id)
			const text = 'Abreu'
