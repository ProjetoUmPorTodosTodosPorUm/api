...
describe('Report Service Integration', () => {
	...
	describe('findOne()', () => {
		...
		it('Should Return a Report', async () => {
			const reportCreated = await createReport(title, shortDescription, month, year, field.id)
	...
	describe('update()', () => {
		it('Should Not Update a Report (Not Found as USER)', async () => {
			try {
				...
				expect(error.response.message).toBe(TEMPLATE.EXCEPTION.NOT_FOUND('relatório', 'o'))
			}
		})
