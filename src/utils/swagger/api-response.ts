...
import { CollectionPoints, MapOptions, PolygonOptions } from '../google-maps/classes'
...
export const ApiResponse = <TModel extends Type<any>>(
	...
	if (options?.paginated) {
		...
		if (options?.extend) {
			;(data.items.allOf[1].properties as any)[options.extend.key] = options.extend.value
		}
		...
		if (options?.extend) {
			;(data.allOf[1].properties as any)[options.extend.key] = options.extend.value
		}
