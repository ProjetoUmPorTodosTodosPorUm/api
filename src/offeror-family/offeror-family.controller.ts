...
export class OfferorFamilyController {
	...
	restore(@Body() restoreDto: RestoreDto, @Jwt() user: User) {
		return this.offerorFamilyService.restore(restoreDto, user)
	}

	@ApiBearerAuth()
	@ApiResponse(CreateOfferorFamilyDto)
	@Put(':id')
	update(@Param('id') id: string, @Jwt() user: User, @Body() updateOfferorFamilyDto: UpdateOfferorFamilyDto) {
	...
	hardRemove(@Body() hardRemoveDto: HardRemoveDto, @Jwt() user: User) {
		return this.offerorFamilyService.hardRemove(hardRemoveDto, user)
	}
