...
describe('Log Controller E2E', () => {
	...
	const createLog = async (ip: string, method: string, url: string, statusCode: string, user: User = null) => {
	...
	beforeAll(async () => {
		...
		admin = await createUser(prisma, 'Sigma', 'admin@example.com', hashedPassword, Role.ADMIN)
